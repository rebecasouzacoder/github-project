...
export class GithubService {
  ...
  public eventError: Subject<boolean> = new Subject<boolean>();
  ...
  public emitEvent(eventName?: string, page?: number): void {
    ...
    this.getUsers(eventName, page).subscribe((result) => {
      if(result.items.length > 0) {
        this.eventSubject.next(result);
        this.emitEventLoading(false)
        this.emitEventError(false)
      } else {
        this.emitEventLoading(false)
        this.emitEventError(false)
      }
  ...
  public emitEventError(error: boolean): void {
    this.eventError.next(error);
  }
  
  public getEventError(): Observable<boolean> {
  ...
  getUsers(search?: string, page?: number ): Observable<IUsersResponse> {
   return  this.http.get<IUsersResponse>(`${environment.api_url}search/users?q=${search ? search : 'q'}&page=${page ? page : 1}&per_page=20`, this.headers);
  }

  getUserById(user: string): Observable<IUserByIdResponse> {
    return  this.http.get<IUserByIdResponse>(`${environment.api_url}users/${user}`, this.headers);
  }
 }